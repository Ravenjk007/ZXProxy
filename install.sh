...
if [ "$EUID" -ne 0 ]; then
    ...
    # ---->>>> Instalar o ZXProxy
    show_progress "Compilando ZXProxy, isso pode levar algum tempo dependendo da maquina..."
    ...
    if [ -f /root/ZXProxy/menu.sh ]; then
        mv /root/ZXProxy/menu.sh /opt/zxproxy/menu
    fi
    ...
    mv ./target/release/vsproxy /opt/zxproxy/proxy || error_exit "Binário compilado não encontrado"
    increment_step
    ...
    if [ -f /opt/zxproxy/menu ]; then
        ...
        ln -sf /opt/zxproxy/proxy /usr/local/bin/"$CMD_NAME"
    fi
    increment_step
